...
- **Verification before committing and pushing**: `scripts/app.sh format` → `scripts/app.sh analyze` → `scripts/app.sh test` → `scripts/app.sh analyze:files` → **build iOS locally**. The PR pipeline only runs analysis and tests; it never builds either app. Always confirm the iOS release build succeeds locally before pushing:
...
- **Rust coverage measures a stripped copy of the crate** (`rust/scripts/coverage.sh`): `frb(...)` attribute macros and `cargo-llvm-cov`'s rustc wrapper each hide code from instrumentation — affected code vanishes from the report rather than counting as uncovered. Hence the temp copy with the attributes deleted, plus `--no-rustc-wrapper`. Remove either and the number silently rises. Don't delete the `frb` attributes for real (codegen breaks) and don't add `pitch_shift` to `[workspace] members` (clippy breaks).
