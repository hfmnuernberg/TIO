...
set +e
SEMVER_TAG=$(git tag --list '*.*.*' --sort=-creatordate | head -n1)
BUILD_NUMBER_TAG=$(git tag --list '*.*.*+.*' --sort=-creatordate | head -n1)
set -e
...
if [ -z "$BUILD_NUMBER_TAG" ]; then
  ...
  BUILD_NUMBER=$(echo "$BUILD_NUMBER_TAG" | sed -E 's/^v?[0-9]+\.[0-9]+\.[0-9]+\+([0-9]+).*/\1/')
