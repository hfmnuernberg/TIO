// Local fork of pitch_shift 1.0.0 (https://github.com/NathanRoyer/pitch_shift)
// Original license: MIT, author: Nathan ROYER <nathan.royer.pro@gmail.com>
//
// Changes from upstream:
// - Fix: frequency bin mapping uses magnitude-weighted averaging instead of overwrite
// - Fix: phase_sum is wrapped to [0, TAU) to prevent f32 precision loss during long playback
            ...
            if self.overlap >= self.frame_size {
                ...
                for k in 0..half_frame_size {
                    ...
                    if index < half_frame_size {
                        ...
                        let freq = k_real * pitch_weight + oversamp_weight * delta_phase;
                        self.synthesized_magnitude[index] += magnitude;
                        self.synthesized_frequency[index] += magnitude * freq;
                    }
                }

                for k in 0..half_frame_size {
                    if self.synthesized_magnitude[k] > 0.0 {
                        self.synthesized_frequency[k] /= self.synthesized_magnitude[k];
                    }
                }
                ...
                let acc_oversamp: SampleReal =
                    2.0 / (half_frame_size * over_sampling) as SampleReal;
