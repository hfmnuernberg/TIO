...
use crate::api::util::util_functions::get_platform_default_cpal_input_config;
...
pub fn tuner_create_stream() -> bool {
    ...
    if config.is_none() {
        log::info!("Could not start tuner input stream - Could not get default input config");
        return false;
    }
...
fn on_audio_callback(data: &[f32], _: &cpal::InputCallbackInfo) {
        ...
        .deref_mut()
    {
        ...
        None => log::info!("Could not lock mutex to RING_PRODUCER"),
    }
}
...
pub fn tuner_trigger_destroy_stream() -> bool {
        ...
        .deref()
    {
        Some(thread_record_sender) => {
            match thread_record_sender.send(CommandTuner::Stop) {
                Ok(_) => {}
                Err(_) => {
                    log::info!("Could not send stop command to tuner thread");
                    return false;
                }
            }
            true
