...
use crate::{
    audio::{audio_buffer::AudioBufferReader, global::GLOBAL_AUDIO_LOCK},
    modules::metronome_rhythm::BeatSound,
    util::{
...
pub fn metronome_create_audio_stream() -> bool {
    ...
    if config.is_none() {
        debug_log("Could not start metronome output stream - Could not get default output config");
        return false;
    }
        ...
        .deref()
    {
        Some(thread_record_sender) => {
            thread_record_sender
                .send(CommandMetro::Stop)
                .expect("Could not send command");
        }
        None => {
            debug_log("Could not send command to destroy metronome audio stream - no sender");
        }
    }
