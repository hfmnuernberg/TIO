...
use crate::audio::audio_buffer_interpolated::AudioBufferInterpolated;
...
pub fn media_player_create_stream() -> bool {
    ...
    debug_log("Starting media player stream");
    ...
    *RING_CONSUMER
    ...
    let audio_out_thread = thread::spawn(move || {
        match device.build_output_stream(
            ...
            Err(e) => debug_log(format!("failed to build audio stream: {}", e).as_str()),
        }
    });
        ...
        Some(ring_consumer) => {
            ...
            for i in 0..samples_out.len() / NUM_CHANNELS {
                match ring_consumer.pop() {
                    ...
                    None => {
                        debug_log("Mediaplayer ring buffer empty - cannot write to audio output");
                        break;
                    }
                }
            }
...
fn pitch_shift(audio_processing_data: &mut AudioProcessingData) {
        ...
        && audio_processing_data.pitch_change_semitones.abs() < EPSILON
    {
        ...
        if let Some(pitch_shifter) = &mut audio_processing_data.pitch_shifter {
            ...
            debug_log("pitch shifter not initialized");
        }
    }
}
